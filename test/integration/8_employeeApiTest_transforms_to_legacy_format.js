...
test('POST /employees', async () => {
    ...
    // and it transforms name to the legacy format
    expect(savedEmployees[0]).to.deep.match({ STR_name: 'John' });
    expect(savedEmployees[0]).to.not.have.key('name');

    // and it transforms jobTitle to the legacy format
    expect(savedEmployees[0]).to.deep.match({ STR_jobTitle: 'Programmer' });
    expect(savedEmployees[0]).to.not.have.key('jobTitle');
